...
//  BattleBoat
//
//  Created by Kylee Down on 1/22/24.
//
...
#include "Game.hpp" // defines Game class
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
m_nGridColumns(k_nGridColumnsDefault), /* ?? add initializers for member variables
m_iRowBoat, m_iColumnBoat, and m_nTorpedoes. ?? */m_iRowBoat(0), m_iColumnBoat(0), m_nTorpedoes(k_nMaxTorpedoes)
{
...
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
/\/\/\/\/\/\/\/\/\/\/\/\/\ //
    for (int iRow = 0; iRow < this->k_nGridRowsDefault; iRow++)
{
    for (int iColumn = 0; iColumn < this->k_nGridColumnsDefault; iColumn++)
{
...
//
// The default random number generator is adequate for games; for betterstatistical
...
// the target boat.
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
/\/\/\/\/\/\/\/\/\/\/\/\/\ // (std::abs(iRow - this->m_iRowBoat) <= 1) && (std::abs(iCol))
...
// from the tuple into iRow and iColumn, respectively.
    if ((iRow < 5)&& (iRow >=0) && (iColumn< 5)&& (iColumn >=0)) {
